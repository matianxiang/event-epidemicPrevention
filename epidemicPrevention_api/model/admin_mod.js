module.exports = class user_mod extends require('./model') {
  ...
  static announceMod(title, classes) {
    return new Promise((resolve, reject) => {
      ...
      this.query(sql, this.formatParams(title, classes)).then((result) => {
        ...
        console.log('发布通知失败:${err.message}') //返回后台
        reject('通知发布失败') //返回页面
      })
    })
  ...
  static getAllNoticeMod(pageNum, currPage) {
    ...
    return new Promise((resolve, reject) => {
      ...
      //  formatParams 封装的把参数转为数组的方法
      // 注意currage,pageNum参数顺序 不能颠倒
      this.query(sql, this.formatParams(currPage, pageNum)).then(result => {
        resolve(result) //返回resul
      }).catch(err => {
        console.log(err)
        reject('分页获取通知失败') //返回错误
      })
    })
  ...
  static getLeaveMod(classArr, currPage, pageNum) {
    ...
    return new Promise((resolve, reject) => {
      ...
      //注意 老师的班级可以有多个 用;隔开 但请假表中的班级一条请假表只能有一个
  ...
  static getuserLeaveToatal(u_id) {
    return new Promise((resolve, reject) => {
      let sql = "select count(1) as count from `leave` where u_id=? order by createtime desc"
  ...
  static addClassesMod(classAll,classes){
     return new Promise((resolve,reject)=>{
          ...
          this.query(sql,this.formatParams(classes)).then(result=>{
              resolve('添加成功')
          }).catch(()=>{
                reject('添加失败')
          })
